...
- **Absence claims require evidence of search.** Before asserting that a mitigation, migration plan, fallback, or alternative DOESN'T exist, state where you looked. "No migration plan exists" requires checking the project's issues, roadmap, or docs. "I did not find a migration plan in the sources reviewed" is honest; "there is no plan" is a claim you must verify. Pre-mortems and Inversion Attacks are especially prone to assuming the absence of mitigations that would weaken the attack — check before asserting.
