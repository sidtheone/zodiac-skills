...
- **Absence claims require evidence of search.** Before asserting that a guardrail, rollback path, or mitigation DOESN'T exist at hop 2 or hop 3, state where you looked. Consequence chains built on false premises ("there is no circuit breaker" when one exists) invalidate the entire chain. Say "I did not find" when you haven't checked, not "there is none." The further down the chain, the more likely you're assuming rather than verifying.
